...
export async function generateMetadata({ params }: TagPageProps): Promise<Metadata> {
  ...
  const tagSlug = paramsData.slug;
  const tag = originalTags.find(t => t.slug === tagSlug);
...
export default async function TagPage({ params, searchParams }: TagPageProps) {
  ...
  // 获取当前标签下的所有文章
  const tagPosts = await getBlogPostsByTag(tagSlug);
  ...
  // 当前标签，使用实际计算的数量
  const currentTag = tagsWithCount.find(t => t.slug === tagSlug);
