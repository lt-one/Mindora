...
### 5. 好站分享功能
- 静态生成（SSG）+ ISR（24小时刷新一次）
- 功能描述：分享对个人有帮助、日常生活能用到的网站，提供分类浏览和搜索功能
- 数据模型：GoodSite（网站名称、URL、描述、分类、标签、图标、添加日期）
- 展示方式：卡片网格布局，按分类和标签筛选
- 交互功能：
  - 分类和标签过滤
  - 搜索功能
  - 网站详情页展示
- 使用Server Components获取数据，客户端组件处理筛选和搜索交互
...
### 7. 观后感文章功能
- 静态生成（SSG）+ ISR（24小时刷新）
- 功能描述：存储读书、观影、追剧等的感想和评论
- 数据模型：Review（标题、内容、类型[书籍/电影/剧集]、评分、创建日期、封面图片）
- 展示方式：卡片列表，按类型分类展示
- 交互功能：
  - 类型筛选
  - 评分排序
  - 时间排序
  - 详情页展示
- 类似博客系统实现，支持Markdown格式内容
- 集成评分组件和媒体信息（可选接入外部API获取书籍/电影数据）
