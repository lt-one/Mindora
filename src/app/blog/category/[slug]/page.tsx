...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  const categorySlug = paramsData.slug;
  const category = originalCategories.find(c => c.slug === categorySlug);
  ...
  return {
    title: `${category.name} - 博客分类 | Mindora`,
    description: category.description || `浏览${category.name}分类下的所有技术文章和教程`,
    openGraph: {
      title: `${category.name} - 博客分类 | Mindora`,
      description: category.description || `浏览${category.name}分类下的所有技术文章和教程`,
      url: `https://Mindora.dev/blog/category/${category.slug}`,
      siteName: "Mindora",
      locale: "zh_CN",
      type: "website",
    },
...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  // 获取当前分类下的所有文章
  const allCategoryPosts = await getBlogPostsByCategory(categorySlug);
  ...
  const tags: BlogTag[] = originalTags.map(tag => {
    // 计算当前分类下具有该标签的文章数量
    const tagPostsInCategory = allCategoryPosts.filter(post => 
      post.tags.some(postTag => postTag.toLowerCase().replace(/ /g, '-') === tag.slug)
    );
    return {
      ...tag,
      count: tagPostsInCategory.length // 用实际数量替换硬编码的数量
    };
  });
  
  const getFilteredPosts = () => {
    let filteredPosts = allCategoryPosts;
    ...
    <main className="min-h-screen relative overflow-hidden bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-950">
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 sm:px-6 lg:px-8 relative z-10">
        <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 mb-6">
          ...
          <span className="text-gray-900 dark:text-white font-medium">分类</span>
          <ChevronRight className="w-4 h-4 mx-2" />
          <span className="text-blue-600 dark:text-blue-400 font-medium">{category.name}</span>
        </div>
        ...
        <div className="mb-8 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-md border border-gray-100 dark:border-gray-700 overflow-hidden">
          <div className="p-6 sm:p-8">
            ...
            {category.image && (
              <div className="aspect-[21/9] w-full overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-700">
                <img
                  src={category.image}
                  alt={category.name}
        ...
        {hasActiveFilters && (
          <div className="max-w-7xl mx-auto mb-4 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <div className="flex items-center text-sm text-blue-700 dark:text-blue-300">
              ...
              <span>在 "{category.name}" 分类下找到 {posts.length} 篇文章</span>
            </div>
          </div>
        )}
        ...
        <section className="max-w-7xl mx-auto relative">
            ...
            {posts.length > 0 ? (
              ...
              <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl p-8 text-center border border-gray-100 dark:border-gray-700">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                  在 "{category.name}" 分类下没有符合当前搜索或筛选条件的文章。请尝试使用不同的关键词或重置筛选条件。
                </p>
                <Link
          ...
          {/* {posts.length === 0 && (
            <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl p-8 text-center border border-gray-100 dark:border-gray-700">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                该分类下暂无文章，或没有符合筛选条件的文章。请尝试更改筛选条件或稍后再查看。
              </p>
              <Link
                href="/blog"
